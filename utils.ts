		...
		networks: [
			{
				...
				genesisTransactions: bcovrinGenesisTransactions,
...
export const initializeFarmerAgent = async () => {
	...
	agent.registerInboundTransport(new HttpInboundTransport({ port: Number(farmerEndpoint.split(":").at(-1)) }));
...
export const initializeLabAgent = async () => {
	...
	agent.registerInboundTransport(new HttpInboundTransport({ port: Number(labEndpoint.split(":").at(-1)) }));
...
export const initializeConsumerAgent = async () => {
	...
	// in mobile agent we should implement relay server instead of this
	agent.registerInboundTransport(new HttpInboundTransport({ port: Number(consumerEndpoint.split(":").at(-1)) }));
