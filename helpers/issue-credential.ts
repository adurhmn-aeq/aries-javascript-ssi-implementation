import { Agent, CredentialEventTypes, CredentialState, CredentialStateChangedEvent, KeyType, TypedArrayEncoder, utils } from "@aries-framework/core";
import { agentModules } from "../utils";
import { labDid, labSeed, staticCredAttrs } from "../constants";
import { IndyVdrRegisterCredentialDefinitionOptions, IndyVdrRegisterSchemaOptions } from "@aries-framework/indy-vdr";
...
export const registerSchema = async (permissionedAgent: Agent<typeof agentModules>) => {
	...
	const { schemaState } = await permissionedAgent.modules.anoncreds.registerSchema<IndyVdrRegisterSchemaOptions>({
...
export const registerCredentialDefinition = async (permissionedAgent: Agent<typeof agentModules>, schemaId: string) => {
	...
	if (credentialDefinitionState.state === "failed") throw new Error(credentialDefinitionState.reason);

	return credentialDefinitionState;
};

export const issueCredential = async (permissionedAgent: Agent<typeof agentModules>, connectionId: string) => {
	...
	const credDef = await registerCredentialDefinition(permissionedAgent, schema.schemaId!);
	...
	console.log("Credential offered by lab agent, now farmer agent should accept it");
	return credExchangeRecord;
};

const acceptCredOffer = async (agent: Agent<typeof agentModules>, credentialRecordId: string) => {
	// prerequisite
...
export const handleIncomingCredOffer = (
	agent: Agent<typeof agentModules>,
	cb: (holderAgent: Agent<typeof agentModules>, credDefId: string) => Promise<void>
) => {
	agent.events.on(CredentialEventTypes.CredentialStateChanged, async ({ payload }: CredentialStateChangedEvent) => {
		if (payload.credentialRecord.state === CredentialState.OfferReceived) {
			console.log("Accepting verification credentials received from lab agent as farmer agent...");
			await acceptCredOffer(agent, payload.credentialRecord.id);
		} else if (payload.credentialRecord.state === CredentialState.Done) {
			cb(agent, payload.credentialRecord.metadata.data["_anoncreds/credential"].credentialDefinitionId);
		}
	});
};
