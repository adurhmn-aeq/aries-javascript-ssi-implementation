...
import { handleAcceptedProofRequest, handleIncomingProofRequest, requestProof } from "./helpers/verify-credential";
import { createNewInvitation, receiveInvitation, setupConnectionListener } from "./helpers/connection";
import { handleIncomingCredOffer, issueCredential } from "./helpers/issue-credential";


const runVerificationFlow = async (holderAgent: Agent<typeof agentModules>, credDefId: string) => {
	...
	const { outOfBandRecord, invitationUrl } = await createNewInvitation(consumerAgent);
	...
	setupConnectionListener(consumerAgent, outOfBandRecord, async (connectionId: string) => {
		console.log("We now have an active connection between consumer agent and a farmer agent");
...
const run = async () => {
	...
	const { outOfBandRecord, invitationUrl } = await createNewInvitation(labAgent);

	// individual invitation has to be listened inorder to connect with the particular agent
	console.log("Listening for connection changes in lab...");
	setupConnectionListener(labAgent, outOfBandRecord, async (connectionId: string) => {
		console.log("We now have an active connection between lab agent and a farmer agent");
